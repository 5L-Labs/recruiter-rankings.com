...
## 2026-02-04 - Unauthorized State Modification in Identity Claim
**Vulnerability:** `ClaimIdentityController#create` updated the `User#linked_in_url` attribute immediately upon receiving a claim request, before any verification took place. This allowed any unauthenticated user to overwrite the LinkedIn URL of any existing user given their email address.
**Learning:** State modification (especially of user profile data) must be deferred until trust is established. Verification flows should not modify the entity's state during the *request* phase, but only during the *confirmation* phase.
**Prevention:** Store pending changes in a temporary verification record (like `IdentityChallenge`) and only apply them to the primary entity (like `User`) upon successful verification.
